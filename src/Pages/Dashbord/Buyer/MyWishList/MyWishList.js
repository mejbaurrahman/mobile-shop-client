...
export default function MyWishList() {
  ...
  return (
    <div>
    <div>
      <h1 className='text-3xl text-primary uppercase text-left'>My Wish List</h1>
...
<table className="table w-full">
  ...
  <tbody>
   {
    wishproducts&& wishproducts?.map((order, i)=> <tr 
    key={order._id}
    className="hover">
    <td>{i+1}</td>
    ...
    <td>{order.price}</td>
    <td><Link to={`/dashbord/pay/${order._id}`}><button className='btn btn-success' disabled={order?.paid}>{order?.paid ? 'Ordered': 'Order'}</button></Link></td>
